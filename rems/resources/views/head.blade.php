<!DOCTYPE html>
<html lang="en">
<head>
  <!-- Title -->
  <title>REMS | {{ucfirst($mode)}}</title>

  <!-- Required Meta Tags Always Come First -->
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

  <!-- Favicon -->
  <link rel="shortcut icon" href="../../favicon.ico">

  <!-- Google Fonts -->
  <link href="//fonts.googleapis.com/css?family=Roboto:300,400,500" rel="stylesheet">

  <!-- CSS Global Compulsory -->
  <link rel="stylesheet" href="{{URL::asset('vendor/bootstrap/css/bootstrap.css')}}">
  <link rel="stylesheet" href="{{URL::asset('vendor/bootstrap/css/bootstrap-social.css')}}">

  <!-- CSS Implementing Plugins -->
  <link rel="stylesheet" href="{{URL::asset('vendor/font-awesome/css/all.min.css')}}">
  <link rel="stylesheet" href="{{URL::asset('vendor/hs-megamenu/src/hs.megamenu.css')}}">
  <link rel="stylesheet" href="{{URL::asset('vendor/malihu-custom-scrollbar-plugin/jquery.mCustomScrollbar.css')}}">
  <link rel="stylesheet" href="{{URL::asset('vendor/custombox/dist/custombox.min.css')}}">
  <link rel="stylesheet" href="{{URL::asset('vendor/animate.css/animate.min.css')}}">
  <link rel="stylesheet" href="{{URL::asset('vendor/slick-carousel/slick/slick.css')}}">
  

  <!-- CSS Space Template -->
  <link rel="stylesheet" href="{{URL::asset('css/theme.css')}}">
  <link rel="stylesheet" href="{{URL::asset('css/specific-styles.css')}}">
</head>